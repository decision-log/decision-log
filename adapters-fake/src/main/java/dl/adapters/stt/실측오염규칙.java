 ...
 */
public final class 실측오염규칙 {

    /** 6회 전부 같은 형태로 깨졌다. */
    public static final Rule 스크럼 = new Rule("스크럼", Mode.일관, List.of("시끄러움"));

    /** 8회 전부 같은 형태. */
    public static final Rule 깃 = new Rule(".git", Mode.일관, List.of("점기"));

    /** 6회 전부 같은 형태. {@link #깃} 과 접두사가 겹친다 — 어댑터가 긴 정답부터 적용한다. */
    public static final Rule 깃이그노어 = new Rule(".gitignore", Mode.일관, List.of("GD 근원"));

    /** 매번 다르게 깨져 같은 말인지도 모른다. 용어 후보의 "반복" 신호에도 안 걸린다. */
    public static final Rule 툴콜링 =
            new Rule("툴 콜링", Mode.흔들림, List.of("툴 풀링", "풀 콜링", "툴 콜딩"));

    public static List<Rule> 전량() {
        return List.of(스크럼, 깃, 깃이그노어, 툴콜링);
    }
